...
I spent a while chasing a question that sounds simple: if I write my logic once
...
three is far more versatile than the other two.
...
The second boundary is a **WebAssembly module loaded in-process**. Instead of a
...
The third boundary is **FFI**: compile the core into a static or dynamic
...
[libsignal](https://github.com/signalapp/libsignal) is the canonical example (a
Rust core exposed to Swift, Java, and TypeScript over a hand-maintained C
boundary), and Mozilla's [UniFFI](https://github.com/mozilla/uniffi-rs)
automates much of that binding generation for Swift, Kotlin, and Python.
...
This is not really a fourth boundary; it is a layer on top of the third one. You
...
burden that makes a libsignal-style boundary expensive to carry.

So protobuf-over-FFI sits neatly between plain FFI (fastest, widest hand-written
...
maintenance-and-safety trade, not a performance one.

## WebAssembly is the most versatile boundary

Of the three, WebAssembly is the one I keep coming back to, because it is the
...
portable enough to carry your shared core there too.

## The platform often decides for you

You do not always get to pick the boundary (the platform picks it for you). On
...
and iOS and Android sit on opposite sides of the line.

iOS forbids spawning a subprocess at all. Its sandbox blocks the traditional
UNIX mechanisms (`fork()`, `exec()`, `posix_spawn()`, `system()`), enforced by
...
Android does not have this restriction. An Android app can spawn child processes
through
...
boundary that is available on all of them.

## Performance, portability, and the cost of maintenance

Picking between the three, when the platform leaves it open, comes down to a
...
would ever collapse the boundary inward.

Portability runs in the opposite direction. FFI is the least portable: you need
...
The third axis, the cost of maintenance, is the one that is easiest to
