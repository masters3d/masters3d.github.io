...
The first thing worth separating is that "thread safety" is really two different
...
Rust guarantees data-race freedom with essentially no runtime concept of an
...
from very different base camps.
...
yours; `&mut T` means you have the only handle that can mutate it right now;
`&T` means you are one of possibly many readers.
...
system will insist on it.

A [`Mutex<T>`](https://doc.rust-lang.org/std/sync/struct.Mutex.html) hands out
access to one writer at a time; a
[`RwLock<T>`](https://doc.rust-lang.org/std/sync/struct.RwLock.html) models the
multiple-readers, single-writer pattern, granting many concurrent read guards
(which behave like `&T`) or a single exclusive write guard (which behaves like
`&mut T`), but never both at once. These types move the "shared XOR mutable"
check to runtime while preserving exactly the same invariant the borrow checker
enforces at compile time, and because they are `Sync` the compiler can then let
the wrapped value be shared safely. The rule never bends; you simply choose
where it is enforced.

On top of that, Rust adds two auto-derived, compositional traits.
[`Send`](https://doc.rust-lang.org/std/marker/trait.Send.html) means a type can
be moved to another thread, and
[`Sync`](https://doc.rust-lang.org/std/marker/trait.Sync.html) means a shared
...
The Rust community has a name for the result of all this working together:
...
becomes a compile error rather than a rare production crash.
...
That care is exactly what shapes the design, and it is where I would gently push
...
starting point,
[`Sendable`](https://developer.apple.com/documentation/swift/sendable) has to
...
because an
[`actor`](https://github.com/swiftlang/swift-evolution/blob/main/proposals/0306-actors.md)
...
strong as the `Sendable`-ness of what crosses it, which is why the actor reads
...
It is worth noticing that Swift's own concurrency proposals describe an actor as
an "island of single-threadedness" with a mailbox as its synchronization point,
echoing the classic [actor model](https://en.wikipedia.org/wiki/Actor_model).
Systems like [Erlang](https://www.erlang.org/) get strong isolation because
...
available and the whole app is a single sandbox.
...
compile-time concurrency safety on top of it through `Sendable`, region
...
require ownership at the core rather than as an opt-in layer.

The interesting part is that the opt-in layer already exists in pieces.
Noncopyable types, `consuming` and `borrowing` ownership, `@Sendable` closures,
