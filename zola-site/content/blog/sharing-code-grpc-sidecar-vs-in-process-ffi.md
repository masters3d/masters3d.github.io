...
The web browser is the other place where you cannot spawn a subprocess, and it has its own in-process answer: compile the shared core to WebAssembly and call it from JavaScript inside the page. This is exactly what 1Password does (they keep their logic in a Rust core and port it to WASM for their web clients rather than re-implementing it in JavaScript). It is the same instinct as the iOS FFI case (one shared core, a thin per-language front end), adapted to a sandbox that only runs WASM. So the two "inline" targets that force you off the sidecar (iOS and the browser) each have a source of truth worth studying:

- Signal's `libsignal` (Rust core exposed to Swift/Java/TypeScript over a C ABI FFI): [https://github.com/signalapp/libsignal](https://github.com/signalapp/libsignal)
- 1Password on using WASM and Rust for local-first web clients (Syntax podcast, episode 776, with Andrew Burkhart): [https://syntax.fm/show/776/how-1password-uses-wasm-and-rust-for-local-first-dev-with-andrew-burkhart/transcript](https://syntax.fm/show/776/how-1password-uses-wasm-and-rust-for-local-first-dev-with-andrew-burkhart/transcript)
