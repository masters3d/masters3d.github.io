...
learning were already in motion).

JavaScript won on the browser by accident. It was never meant to be the lingua
...
the web, you bet I'll use TypeScript.

After Python, I learned Swift. Apple had just released it as the new language
...
Swift had taken a lot of ideas from Go (the `defer` keyword, the `func` naming
convention, the overall simplicity).

Most of my professional career so far has been C#, Go, Python, C++, and Rust.
...
Now that we're in this world of LLM-driven development, the language choice
...
trade-offs). Here's my decision matrix, broken down by deployment target.

**Frontend (Visual UI for end users):**

- **Web:** TypeScript
- **Mobile Android:** Kotlin (the modern replacement for Java, though I wouldn't
  rewrite existing Java to Kotlin)
- **Mobile iOS:** Swift
- **Desktop Windows:** C#
- **Desktop macOS:** Swift
- **Desktop Linux:** Rust
...
This is the bridge most people miss. CLI tools are more portable than visual
...
executables). For actual CLI tools, assume cross-platform only.
...
The rule of thumb: If the CLI will grow beyond a few hundred lines or needs
...
- **REST APIs, web services, large teams:** C# (ASP.NET Core) or Go. C# is not a
...
The rule of thumb: If you need to stand up a server with REST endpoints, talk to
...
lack of static typing and the deployment complexity make it less ideal.

As you go down the stack and have to run on smaller, more constrained
...
for the glue and the high-level logic.

## The LLM Era: Even Coding Agents Show This Diversity

The equation does change when it comes to AI. LLMs are able to write Rust just
...
for me (but now by choice, I use other languages instead).

Interestingly, even coding agents themselves show this diversity in language
choice:

- **[Codex CLI](https://github.com/openai/codex):** Written in Rust
...
The diversity is striking. While Aider does use Python, most newer coding agents
...
well-understood deployment story.

I don't think I'm picky when it comes to languages. I just probably don't see
...
compiled output. That's fine.

No, I'm not going to port anything to Rust (or Kotlin, or any other language)
...
language. That's my high-level thinking.

I am very excited about Mojo, which is meant to bridge the gap between Python
...
compile-time language for most cases.

What matters now isn't just syntax or learning curve (it's the entire
...
chosen because of its different capabilities).

We've seen this play out in real projects. The Ladybird browser initially
